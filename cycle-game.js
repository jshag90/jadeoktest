...
let obstacleSpawnInterval = 1500; // 초기 스폰 간격 (1.5초)
let minObstacleSpawnInterval = 800; // 최소 스폰 간격 (0.8초)
let lastObstacleSpawn = 0; // 마지막 장애물 생성 시간
...
function updateObstacleSpawn() {
  ...
  if (currentTime - lastObstacleSpawn >= obstacleSpawnInterval) {
    ...
    // 점수가 높아질수록 생성 간격 감소
    obstacleSpawnInterval = Math.max(
      minObstacleSpawnInterval,
      1500 - Math.floor(score / 100) * 100
    );
  }
}
...
function update() {
  ...
  if (keys["ArrowRight"] && bike.x + bike.width < canvas.width - 60)
    bike.x += moveSpeed;
  ...
  // 충돌 체크 (보호막이 있으면 무시)
  if (!gameOver && shieldTime < 1) {
    obstacles.forEach((obs) => {
      if (
        bike.x < obs.x + obs.width - 10 &&
        bike.x + bike.width - 10 > obs.x &&
        bike.y < obs.y + obs.height - 10 &&
        bike.y + bike.height - 10 > obs.y
      ) {
        gameOver = true; // 한 번만 실행
        //alert("게임 종료! 점수: " + Math.floor(score));
        showScoreModal(Math.floor(score)); // 모달 표시
        return;
  ...
  powerGels.forEach((p, index) => {
    if (
      bike.x < p.x + p.width &&
      bike.x + bike.width > p.x &&
      bike.y < p.y + p.height &&
      bike.y + bike.height > p.y
    ) {
      const points = 10;
      score += points;
  ...
  waterBottles.forEach((w, index) => {
    if (
      bike.x < w.x + w.width &&
      bike.x + bike.width > w.x &&
      bike.y < w.y + w.height &&
      bike.y + bike.height > w.y
    ) {
      const points = 5;
      score += points;
  ...
  if (timeLeft <= 0 && !gameOver) {
    gameOver = true;
    alert("시간 종료! 최종 점수: " + Math.floor(score));
    document.location.reload();
    return;
  }
...
function draw() {
  ...
  ctx.font = "14px Arial";
  const displaySpeed =
    powerBoostTime > 0 ? (gameSpeed * 2.5).toFixed(1) : gameSpeed.toFixed(1);
  ...
  // 남은 시간 - 빨간색으로 표시하고 깜빡이는 효과 추가
  if (timeLeft <= 10) {
    ctx.fillStyle = "#ff0000";
    if (Math.floor(timeLeft * 2) % 2 === 0) {
      ctx.globalAlpha = 0.5;
    }
  }
  ctx.strokeText("남은 시간: " + Math.ceil(timeLeft) + "초", 10, 80); // "초" 추가
  ctx.fillText("남은 시간: " + Math.ceil(timeLeft) + "초", 10, 80); // "초" 추가
  ctx.globalAlpha = 1;
  ...
  if (powerBoostTime > 0) {
    ...
    // 위치 변경
    ctx.strokeText("⚡ SPEED BOOST! ⚡", canvas.width - 180, 30);
    ctx.fillText("⚡ SPEED BOOST! ⚡", canvas.width - 180, 30);
    ...
    const gaugeX = canvas.width - 130; // 오른쪽에서부터 위치 계산
    const gaugeY = 35;
  ...
  if (shieldTime >= 1) {
    ...
    ctx.strokeText("🛡️ SHIELD ACTIVE! 🛡️", canvas.width - 180, 70);
    ctx.fillText("🛡️ SHIELD ACTIVE! 🛡️", canvas.width - 180, 70);
    ...
    const shieldGaugeX = canvas.width - 130;
    const shieldGaugeY = 75;
...
// 수통은 3.5초마다 생성
setInterval(spawnWaterBottle, 6000);
...
scoreForm.addEventListener("submit", function (e) {
  ...
  // 여기에 실제 점수 저장 로직을 추가하세요
  // 예: localStorage, 서버 API 호출 등
  saveScore(nickname, score);

  hideScoreModal();
  alert(`${nickname}님의 점수 ${score}점이 등록되었습니다!`);
});
